...
| Code | pre-code until spec 002 lands; then the chassis dirs (`addon/`, `core/`, `auth/`, `idp/`, `lib/`, `hiq/`, `health/`, `web/`) plus the services (`tenants/`, `factory/`, `fleet/`, `webapp/`) | after spec 002: `npm run typecheck` + `npm test` (the chassis gates) |
