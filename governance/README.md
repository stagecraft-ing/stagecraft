...
config/gate.v1.json the committed gate config (hash pinned by an addon test)
```

The heavy lifting is in the `governance-native` napi-rs addon
(`addon/governance-native/`), which wraps four crates.io crates:
...
The addon is complete and its Rust tests pass
(`cargo test --no-default-features` in `addon/governance-native/`, including the
