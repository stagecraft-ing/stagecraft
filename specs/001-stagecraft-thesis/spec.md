...
establishes:
  - "README.md"
summary: >
  The product thesis and the consolidation record. Stagecraft is the
...
Stagecraft sells one loop: **intent -> governed spec -> factory-stamped
...
Stagecraft is the first production EnRaHiTu app, deliberately: every fleet
...
user logs in via OAuth, creates a tenant, installs the Stagecraft GitHub
...
template code and must be unencumbered) and stagecraft-cli is Apache-2.0
...
- **M4: the agent bridge.** stagecraft-cli's MCP server lets an agent
...
- CLI/MCP implementation (stagecraft-cli repo).
